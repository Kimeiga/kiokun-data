...
export class CardManager {
  ...
  public combineCards(hand: Hand, cardId1: string, cardId2: string, chosenChar?: string): Card | null {
    ...
    // If a specific character was chosen, use it (must be in possibleResults)
    let resultChar: string;
    if (chosenChar && possibleResults.includes(chosenChar)) {
      resultChar = chosenChar;
    } else {
      // Default to first result
      resultChar = possibleResults[0];
    }
  ...
  // Get possible combination results without actually combining
  public getPossibleCombinations(char1: string, char2: string): string[] {
    return this.dataLoader.canCombine(char1, char2);
  }
